...
//        REGION);
      ...
      KinesisClientLibConfiguration config = new KinesisClientLibConfiguration(APPLICATION_NAME, STREAM_NAME, credentialsProvider, workerId);
      config.withRegionName(REGION.toString());
      config.withInitialPositionInStream(APPLICATION_INITIAL_POSITION_IN_STREAM);
      return config;
  }
  ...
  private static Worker createWorker(KinesisClientLibConfiguration config) {
    // STUDENT TODO 8: Replace the solution with your own code
	  SensorRecordProcessorFactory factory = new SensorRecordProcessorFactory();
	  final Worker worker = new Worker(factory, config);
	  return worker;
